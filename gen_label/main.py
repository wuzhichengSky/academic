...
IMAGE_ROOT = r"D:\dataset-w\SUES-200-512x512-V2\SUES-200-512x512\satellite-view"
#IMAGE_ROOT = "./data/SUES-200/drone_view_512"
OUTPUT_JSON = "satellite-new.json"
ERROR_LOG = "error.log"
...
def collect_images_satellite(root):
    ...
    for i in range(1, 201):
        scene_id = f"{i:04d}"
        img_path = os.path.join(root, scene_id, "0.png")
        ...
        if os.path.exists(img_path):
            ...
            # 如果缺失，可以选择记录或忽略
            with open(ERROR_LOG, "a", encoding="utf-8") as log:
                log.write(f"MISSING: {img_path}\n")

    # 场景本身已经是有序的，这里排序可留可不留
    imgs = sorted(imgs, key=natural_key)
...
if __name__ == "__main__":
    ...
    for img_path in tqdm(images):
        ...
        key_path = img_path.split("satellite-view/", 1)[-1]
        key_path = "satellite-view/" + key_path
